...
- Open WebUI: `0.11.2` in einer getrennten Python-3.11-Umgebung
...
```text
ghcr.io/falkzilm/qwen-runpod-helper:0.1.0
```
...
1. Öffne die
   ...
   melde dich an und akzeptiere die Nutzungsbedingungen.
2. Erstelle in Hugging Face unter **Settings → Access Tokens** einen Token mit
   ausschließlich Leserechten.
...
| Container Image | `ghcr.io/falkzilm/qwen-runpod-helper:0.1.0` oder eigener Tag |
...
1. Hugging Face lädt die Modell-Shards.
2. vLLM reserviert Gewichtsspeicher und KV-Cache.
3. Open WebUI startet intern auf Port 8081.
4. Nginx veröffentlicht Port 8080.
...
Die Modellbedingungen wurden nicht mit demselben Account akzeptiert, der
`HF_TOKEN` erstellt hat, oder der Token wurde falsch übertragen. Einen neuen
read-only Token erstellen und das private Template aktualisieren.
...
`VLLM_INTERNAL_API_KEY` unverändert im Template vorhanden ist. Beides setzt das
...
Geprüft werden Shell-Syntax und, sofern vorhanden, die Rechte und Platzhalter
der lokalen Secret-Datei. Ein Image-Build benötigt Docker und Internetzugriff;
ein Modelltest benötigt eine passende NVIDIA-GPU.
